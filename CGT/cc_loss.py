...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        elif labels is not None: # here
            labels = labels.contiguous().view(-1, 1)
            if labels.shape[0] != batch_size:
                raise ValueError('Num of labels does not match num of features')
            
            mask = torch.eq(labels, labels.T).float().to(device)
            mask[:, ori_labels==766] = 0 # 766 or 37
        else:
            mask = mask.float().to(device)

        contrast_count = features.shape[1] # features: [100, 1, 512]
        contrast_feature = torch.cat(torch.unbind(features, dim=1), dim=0) # contrast_feature: [100, 512]
        if self.contrast_mode == 'one':
            anchor_feature = features[:, 0] # anchor_feature: [100, 512]
            anchor_count = 1
        ...
        anchor_dot_contrast = torch.div(torch.matmul(anchor_feature, contrast_feature.T), 10)
        ...
        logits_mask[:, ori_labels==766] = 0 # 766 or 37
