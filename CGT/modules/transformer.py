...
def multi_head_attention_forward(query,                           # type: Tensor
    ...
    if not use_separate_proj_weight: ### here
        if torch.equal(query, key) and torch.equal(key, value): ### here
            # self-attention
    ...
    if attn_mask is not None: ### not into here
    ...
    if bias_k is not None and bias_v is not None: ### not into here
    ...
    if add_zero_attn: # not into here
        src_len += 1
...
class TransformerEncoder(Module):
    ...
    def forward(self, src, f_mask, mask=None, src_key_padding_mask=None):
        ...
        for i, mod in enumerate(self.layers):
            output = mod(output, f_mask, src_mask=mask, src_key_padding_mask=src_key_padding_mask)
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, 
        ...
        self.self_cross_attn = MultiheadAttention(d_model, nhead, dropout=dropout)
    ...
    def forward(self, src, f_mask, src_mask=None, src_key_padding_mask=None):
        # type: (Tensor, Tensor, Optional[Tensor], Optional[Tensor]) -> Tensor
        ...
        src2, attn = self.self_cross_attn(f_mask, src, src, attn_mask=src_mask, key_padding_mask=src_key_padding_mask)
...
# class TransformerEncoderLayer_cross_only(Module):
#     r"""TransformerEncoderLayer is made up of self-attn and feedforward network.
#     This standard encoder layer is based on the paper "Attention Is All You Need".
#     Ashish Vaswani, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N Gomez,
#     Lukasz Kaiser, and Illia Polosukhin. 2017. Attention is all you need. In Advances in
...
#     def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, 
#                  activation="relu", debug=False):
#         super(TransformerEncoderLayer_cross_only, self).__init__()
#         self.debug = debug
#         # self.self_attn = MultiheadAttention(d_model, nhead, dropout=dropout)
#         self.self_cross_attn = MultiheadAttention(d_model, nhead, dropout=dropout)
...
#         if 'activation' not in state:
#             state['activation'] = F.relu
#         super(TransformerEncoderLayer_cross_only, self).__setstate__(state)
